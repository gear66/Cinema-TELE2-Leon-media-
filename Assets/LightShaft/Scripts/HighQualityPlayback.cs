...
public class HighQualityPlayback : MonoBehaviour {
    ...
    public void PlayYoutubeVideo(string _videoId)
	{
        ...
        Debug.Log("PlayYoutubeVideo called vvv");
	...
	{
        Debug.Log("FinishLoadingUrls called vvv");
        ...
        audioDecryptDone = false;
            ...
            if (!foundVideo && quality == 2160)
            {
                foreach (VideoInfo info in videoInfos)
                {
                    ...
                    {
                        Debug.Log("Found but with unknow format in results, check to see if the video works normal. vvv");
                        if (info.RequiresDecryption)
                        {
                            //The string is the video url
                            StartCoroutine(resolver.DecryptDownloadUrl(DecryptVideoDone, info));
            ...
            if (!foundVideo)
            {
                Debug.Log("Desired quality not found, playing with low quality, check if the video id: " + videoId + " support that quality! vvv");
                foreach (VideoInfo info in videoInfos)
                {
                    if (info.VideoType == VideoType.Mp4 && info.Resolution == (360))
                    {
    ...
    public void DecryptAudioDone(string url)
    {
        ...
        Debug.Log("DecryptAudioDone called vvv");
        if (videoDecryptDone)
            videoAreReadyToPlay = true;
    }
    ...
    {
        Debug.Log("DecryptVideoDone called vvv");
        videoUrl = url;
    ...
    void FixedUpdate(){
        ...
        if (Input.GetKeyUp(KeyCode.P))
        {
            ...
            //Debug.Log("SwitchToLocal called with: " + lastFrameReady + " " + GetComponent<VideoPlayer>().time + " " + GetComponent<VideoController>().currentTimeString);
            engine.GetComponent<engineClient>().SwitchToLocal(lastFrameReady, GetComponent<VideoPlayer>().time);
        }
	...
	void AudioPrepared(VideoPlayer vPlayer){
        Debug.Log("AudioPrepared called vvv");
        audioVplayer.prepareCompleted -= AudioPrepared;
        audioPrepared = true;
	...
	void VideoPrepared(VideoPlayer vPlayer){
        Debug.Log("VideoPrepared called vvv");
        unityVideoPlayer.prepareCompleted -= VideoPrepared;
        videoPrepared = true;
		if (noHD) {
            Play();
    ...
    public void OnVideoFinished(){
		if (unityVideoPlayer.isPrepared) {
            ...
            gameObject.GetComponent<VideoController>().GoCinemaBl();
        }
	}
    ...
    private void CheckIfIsDesync(){
        if (desyncFlag)
        {
            ...
            {
                Debug.Log("Dropping frames, retry with low quality!: " + lastFrameReadyTime + " vvv " + lastFrameReady);
            ...
            {
                Debug.Log("Starting time out, retry with low quality!: " + lastFrameReadyTime + " vvv " + lastFrameReady);
                //lastTryQuality = VideoQuality.mediumQuality;
                //RetryPlayYoutubeVideo();
                GetComponent<VideoPlayer>().Pause();
                engine.GetComponent<engineClient>().SwitchToLocal(lastFrameReady, GetComponent<VideoPlayer>().time);
            ...
            {
                Debug.Log("Error detected!, retry with low quality! vvv");
                //lastTryQuality = VideoQuality.mediumQuality;
                //RetryPlayYoutubeVideo();
                engine.GetComponent<engineClient>().SwitchToLocal(lastFrameReady, GetComponent<VideoPlayer>().time);
                desyncFlag = false;
    ...
    private void Sync()
    {
        ...
        {
            Debug.LogWarning("Please add a video controller to your scene to make the sync work! Will be improved in the future. vvv");
        }
    }
    ...
    {
        Debug.Log("IEnumerator NewRequest called vvv");
    ...
    public void RetryPlayYoutubeVideo()
    {
        ...
        {
            StopIfPlaying();
            Debug.Log("Youtube Retrying...: vvv" + lastTryVideoId);
            isRetry = true;
            if (this.GetComponent<VideoController>() != null)
            {
                this.GetComponent<VideoController>().ShowLoading("Loading... vvv");
            }
    ...
    private void VideoStarted(VideoPlayer source)
    {
        ...
        Debug.Log("Youtube Video Started vvv");
    }
    ...
    private void VideoErrorReceived(VideoPlayer source, string message)
    {
        ...
        Debug.Log("Youtube VideoErrorReceived!: vvv " + message);
    }
